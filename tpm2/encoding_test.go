...
func TestEncodeCreate(t *testing.T) {
	...
	cmdBytes, err := encodeCreate(
		HandleOwner, pcrSelection, "", defaultPassword,
		[]byte{255} /*sensitiveData*/, params)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(cmdBytes, testCmdBytes[10:]) {
...
func TestEncodeUnseal(t *testing.T) {
	...
	cmdBytes, err :=
		encodeUnseal(HandlePasswordSession, tpmutil.Handle(0x80000001), defaultPassword)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(cmdBytes, testCmdBytes[10:]) {
